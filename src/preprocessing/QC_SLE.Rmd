...
```{r}
raw.molecules <- fread("./dat/SLE/SDY997/ResultFiles/RNA_sequencing_result/celseq_matrix_ru10_molecules.tsv.gz", header=T, sep="\t") %>% as.data.frame()
...
meta <- read_tsv("./dat/SLE/SDY997/ResultFiles/RNA_sequencing_result/celseq_meta.tsv.gz", col_types = cols()) %>% as.data.frame()
```
...
keep_cells <- scset@colData$type!="Empty"
scset <- scset[keep_feature, keep_cells]
...
```{r}
saveRDS(filtered_scset, "./res/SLEcelseq/full_scset_large.rds")
```
...
dev_filtered_scset <- filterDev(genes_filtered, nkeep=3000, dev = "binomial")
```
...
saveRDS(dev_filtered_scset, "./res/SLEcelseq/deviance_scset_large.rds")
```

```{r}
write_feather(as.data.frame(t(dev_filtered_scset@assays$data$counts)), "./res/SLEcelseq/deviance_counts_large.feather")
write_feather(as.data.frame(dev_filtered_scset@colData), "./res/SLEcelseq/deviance_meta_large.feather")
write_feather(as.data.frame(rowData(dev_filtered_scset)), "./res/SLEcelseq/deviance_rowdata_large.feather")
```

```{r}
imputed.values <- fread("./res/SLEcelseq/scVI_4-7-19_30latent_imputed_large.tsv", header=T, sep="\t") %>% as.data.frame()
